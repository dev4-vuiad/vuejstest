                            ...
                            <div class="vodi-archive-wrapper" data-view="grid">
                                <div class="tv-shows columns-6">
                                    <div class="tv-shows__inner">
                                        <TvshowsItem v-for="(item, index) in data.data.items" :key="index" :year="item.year" :title="item.title" :tvshowTitle="item.tvshowTitle" :originalTitle="item.originalTitle" :episodeNumber="item.episodeNumber" :seasonNumber="item.seasonNumber" :postDateGmt="item.postDateGmt" :src="item.src" :chanelImage="item.chanelImage" />
                                    </div>
                                </div>
                            </div>
                            <Pagination v-if="data && data.total > data.perPage" base="/tv-shows" :perPage="data.perPage" :currentPage="page" :total="data.total" :orderBy="orderBy" />
                        </div><!-- /.content-area -->
