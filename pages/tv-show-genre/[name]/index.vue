<script setup>
    ...
    useHead({
        title: 'Full HD ' +  data && data.data && data.data.items.length && data.data.items[0].genres ? (getGenreName(genre, data.data.items[0].genres) || genre) + ' 다시보기 - BMyTV.com (비마이티비) 최신 영화, 드라마, 예능, 미드를 초고화질 링크를 제공하는 무료 다시보기' : '',
        meta: [
            {
                hid: 'description',
                name: 'description',
                content: 'Full HD ' + data && data.data && data.data.items.length && data.data.items[0].genres ? (getGenreName(genre, data.data.items[0].genres) || genre) + 
                ' 다시보기 - BMyTV.com (비마이티비) 최신 한국영화, 서양영화, 동양영화, 한국드라마, 예능, 시사, 미드, 일드 OTT (넷플릭스, 와챠, 디즈니플러스, 애플티비, 아마존 프라임 비디오, 티빙 등) | 컨텐츠를 초고화질(Full HD) 링크로 제공.' : ''
            }
        ],
        meta: [
            {
            ...
            <div class="container">
                ...
                <div class="site-content__inner">
                    <div id="primary" class="content-area" v-if="data"> <!-- ads tv-show top -->
                        ...
                        <Pagination v-if="data && data.total * 1 > data.perPage * 1" :base="'/tv-show-genre/' + genre" :perPage="data.perPage" :currentPage="page" :total="data.total" @on-select-page="onSelectPage" />
