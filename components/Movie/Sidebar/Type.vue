<script setup>
    ...
    const props = defineProps(['selected', 'year', 'page'])
...
<template>
    <ul class="masvideos-widget-movies-layered-nav-list">
        <li v-for="(genre, index) in genres" :key="index" :class="'masvideos-widget-movies-layered-nav-list__item masvideos-layered-nav-term ' + (selected.includes(genre) ? 'masvideos-widget-movies-layered-nav-list__item--chosen chosen' : '')">
            <a rel="nofollow" :href="'/movie' + (typeof page != 'undefined' ? '/page/' + page : '') + '?query_type_genre=or&filter_genre=' + (selected.includes(genre) ? selected.filter(v => v != genre) : selected.concat(genre).join(',')) + (year.length ? '&year_filter=' + year : '')">
                {{ genre }}
            </a>
        </li>
    </ul>
